...
abstract class AiModelview extends ConsumerState<AiScreen>
  ...
  Future<void> refreshMessage(AskNotifier chatRef) async {
    if (chatRef.isTyping) {
  ...
  Future<void> sendMessage(AskNotifier chatRef) async {
    ...
    try {
      ...
      await chatService?.addMessage(msg: msg, role: ResponseTypeEnum.user.name);
      setState(() {
  ...
  Future<void> sendMessageandGetAnswers({
    ...
    final answers = await ChatService(service).fetchChatResponse(msg, basePath);
